...
const IndividualData = ({ url, userData }) => {
  ...
  return (
    <a href={userData.html_url} target="_blank" className="innerFlex-container" rel="noreferrer">

      <div className="innerFlex-items">
        <img className="userAvatar" src={userData.avatar_url} alt="avatar"></img>
      </div>

      <div className="innerFlex-items">
        {gitUser.name ? (
            <h1 className="trueName">{gitUser.name} <small className="loginName">({gitUser.login})</small></h1>
        ) : (
            <h1 className="trueName">No Name Set <small className="loginName">({gitUser.login})</small></h1>
        )}
